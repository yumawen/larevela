...
INSERT INTO payment_transactions (
  ...
  from_token_account, to_token_account, asset_address, asset_symbol, amount_actual, tx_status, confirmations,
  confirmation_status, reference_id, created_at, updated_at
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'success', 1, 'confirmed', ?, UTC_TIMESTAMP(), UTC_TIMESTAMP())
ON DUPLICATE KEY UPDATE
  ...
  from_token_account = VALUES(from_token_account),
  to_token_account = VALUES(to_token_account),
...
INSERT INTO payment_intents (
  ...
  payer_token_account, receiver_token_account, asset_symbol, asset_address, amount_expected, decimals, reference_id, calldata, quote_expired_at,
  confirmations, confirmation_status, status, created_at, updated_at
) VALUES (?, ?, ?, ?, ?, 'transfer', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'pending', ?, UTC_TIMESTAMP(), UTC_TIMESTAMP())
ON DUPLICATE KEY UPDATE
  payer_account = VALUES(payer_account),
  receiver_account = VALUES(receiver_account),
  payer_token_account = VALUES(payer_token_account),
  receiver_token_account = VALUES(receiver_token_account),
...
`
	...
	_, err := m.conn.ExecCtx(
		...
		in.PayerAccount,
		in.ReceiverAccount,
		in.PayerTokenAccount,
		in.ReceiverTokenAccount,
		in.AssetSymbol,
		in.AssetAddress,
		in.AmountExpected,
		in.Decimals,
...
func (m *Model) GetPaymentIntent(ctx context.Context, paymentNo string) (*PaymentIntent, error) {
	...
	var row struct {
		...
		PayerTokenAccount    string `db:"payer_token_account"`
		ReceiverTokenAccount string `db:"receiver_token_account"`
...
SELECT payment_no,
       ...
       IFNULL(payer_token_account, '') AS payer_token_account,
       IFNULL(receiver_token_account, '') AS receiver_token_account,
...
`
	...
	return &PaymentIntent{
		...
		PayerTokenAccount:    row.PayerTokenAccount,
		ReceiverTokenAccount: row.ReceiverTokenAccount,
